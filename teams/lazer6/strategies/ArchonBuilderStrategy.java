...
public class ArchonBuilderStrategy extends Strategy {
	...
	private final int WANDER_TIMEOUT = 100;
	...
	public void runBehaviors() {
		//player.myRC.setIndicatorString(2, ""+buildState);
		...
		if (Clock.getRoundNum() > 500) {
			//if we have not seen an enemy for WANDER_TIMEOUT rounds, switch state to build hull (permanent switch for now)
			//COMMENT OUT THIS CHANGE OF STATE TO STATE2 IF WANT TO REVERT TO OLD BUILDING
			if (myProfiler.enemyAttackersInRange + myProfiler.enemyAirInRange == 0) {
				roundsOfNoEnemy++;
			} else {
				roundsOfNoEnemy = 0;
			}
			if (roundsOfNoEnemy >= WANDER_TIMEOUT) {
				//state = 2;
				buildMode = true;
			}
		}
