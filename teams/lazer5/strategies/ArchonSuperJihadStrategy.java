...
public class ArchonSuperJihadStrategy extends Strategy {
	...
	public ArchonSuperJihadStrategy(RobotPlayer player) {
		super(player);
	...
	public void runBehaviors() throws GameActionException {
		...
		if (currID == 0 || currID == 1) {
			...
			//player.changeStrategy(new ArchonChainerStrategy(player));
		}
		...
		if (player.myRC.getLocation().distanceSquaredTo(atkLoc) < 16 && chargingSoldier == true) {
			archonMoveRandom();
		} else {
