...
public class ArchonStartDefStrategy extends Strategy {
	private int state = 0;
	private Direction rushDir = player.myRC.getDirection();
	private Direction spreadDir;
	private final double SPAWN_THRESHOLD = 50.0;
	
	private Behavior spawnSoldier, spawnWout, Charge, spawnChainer;
	...
	private static final int RUSH_DISTANCE = 150;
	
	public ArchonStartDefStrategy(RobotPlayer player) {
		super(player);
		spawnSoldier = new SpawnSoldierBehavior(player);
		spawnChainer = new SpawnChainerBehavior(player);
...
//				}				
			...
			numRounds++;
			if (numRounds >= 50 && Charging == false) {
				state = 5;
			}
			break;
		case 5:
			...
			if (distanceFromOrigin > RUSH_DISTANCE) {
				if (ID==0 || ID==1) {
